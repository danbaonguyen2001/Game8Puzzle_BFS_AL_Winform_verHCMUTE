...
namespace ProjectAI_Game8Puzzle_BFS
{
    class Solve
    {
		...
		public Solve(State trangThaiDau, State trangThaiDich)
		{
		...
		public List<State> Solve_BreadthFS() //Breadth first search
		{
			dem = 0;
			...
			while (danhSachDinhDuyet.Count > 0)
			{
				...
				List<State> huongPhanTachTrangThai = ptLayRa.PhanTachTrangThai();
				foreach (var item in huongPhanTachTrangThai)
				{
					if (item.KiemTraDenDich(trangThaiDich) == true)
					{
						Console.WriteLine("Ban da chien thang!.....");
						//truy vết lúc này sẽ trả về danh sách đường đi nhưng bị ngược => qua nút Giải reverse nó
						LanVet(ketQua, item);
		...
		//truy vết để đưa mỗi nút cha đã gán trước đó vào trong list kết quả
